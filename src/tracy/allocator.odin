...
MakeProfiledAllocator :: proc(
	...
	secure: b32 = true,
	allocator:=context.allocator) -> mem.Allocator {

	self := new(ProfiledAllocatorData, allocator)

	self.callstack_size = callstack_size
	self.secure = secure
	self.backing_allocator = backing_allocator
	self.allocator = allocator
	self.allocations = make(map[rawptr]bool, allocator)

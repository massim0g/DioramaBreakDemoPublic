...
CHECKVERSION :: proc() {
	DebugCheckVersionAndDataLayout(VERSION, size_of(IO), size_of(Style), size_of(Vector2), size_of(Vec4), size_of(DrawVert), size_of(DrawIdx))
}
...
WindowFlag :: enum c.int {
	...
	HorizontalScrollbar       = 11, // Allow horizontal scrollbar to appear (off by default). You may use SetNextWindowContentSize(ImVector2(width,0.0f)); prior to calling Begin() to specify width. Read code in imgui_demo in the "Horizontal Scrolling" section.
...
Style :: struct {
	...
	WindowMinSize:              Vector2,            // Minimum window size. This is a global setting. If you want to constrain individual windows, use SetNextWindowSizeConstraints().
	...
	CellPadding:                Vector2,            // Padding within a table cell. CellPadding.y may be altered between different rows.
	...
	ButtonTextAlign:            Vector2,            // Alignment of button text when button is larger than text. Defaults to (0.5f, 0.5f) (centered).
	SelectableTextAlign:        Vector2,            // Alignment of selectable text. Defaults to (0.0f, 0.0f) (top-left aligned). It's generally important to keep this left-aligned if you want to lay multiple items on a same line.
	SeparatorTextBorderSize:    f32,             // Thickkness of border in SeparatorText()
	SeparatorTextAlign:         Vector2,            // Alignment of text within the separator. Defaults to (0.0f, 0.5f) (left aligned, center).
	SeparatorTextPadding:       Vector2,            // Horizontal offset of text from each edge of the separator + spacing on other axis. Generally small values. .y is recommended to be == FramePadding.y.
	DisplayWindowPadding:       Vector2,            // Window position are clamped to be visible within the display area or monitors by at least this amount. Only applies to regular windows.
	DisplaySafeAreaPadding:     Vector2,            // If you cannot see the edges of your screen (e.g. on a TV) increase the safe area padding. Apply to popups/tooltips as well regular windows. NB: Prefer configuring your TV sets correctly!
...
IO :: struct {
	...
	DisplaySize:             Vector2,         // <unset>          // Main display size, in pixels (generally == GetMainViewport()->Size). May change every frame.
	...
	DisplayFramebufferScale: Vector2,         // = (1, 1)         // For retina display or other situations where window coordinates are different from framebuffer coordinates. This generally ends up in ImDrawData::FramebufferScale.
	...
	MouseDelta:                 Vector2,                // Mouse delta. Note that this is zero if either current or previous position are invalid (-FLT_MAX,-FLT_MAX), so a disappearing/reappearing mouse won't have a huge delta.
	...
	MousePos:    Vector2,        // Mouse position, in pixels. Set to ImVector2(-FLT_MAX, -FLT_MAX) if mouse is unavailable (on another screen, etc.)
	...
	MousePosPrev:                     Vector2,               // Previous mouse position (note that MouseDelta is not necessary == MousePos-MousePosPrev, in case either position is invalid)
	MouseClickedPos:                  [5]Vector2,            // Position at time of clicking
...
SizeCallbackData :: struct {
	...
	Pos:         Vector2,   // Read-only.   Window position, for reference.
	CurrentSize: Vector2,   // Read-only.   Current window size.
	DesiredSize: Vector2,   // Read-write.  Desired size, based on user's mouse position. Write to this field to restrain resizing.
}
...
DrawList :: struct {
	...
	_Path:           Vector_Vector2,         // [Internal] current path building
...
DrawData :: struct {
	...
	DisplayPos:       Vector2,               // Top-left position of the viewport to render (== top-left of the orthogonal projection matrix to use) (== GetMainViewport()->Pos for the main viewport, == (0.0) in most single-viewport applications)
	DisplaySize:      Vector2,               // Size of the viewport to render (== GetMainViewport()->Size for the main viewport, == io.DisplaySize in most single-viewport applications)
	FramebufferScale: Vector2,               // Amount of pixels for each unit of DisplaySize. Based on io.DisplayFramebufferScale. Generally (1,1) on normal display, (2,2) on OSX with Retina display.
...
FontConfig :: struct {
	...
	GlyphExtraSpacing:    Vector2,   // 0, 0     // Extra spacing (in pixels) between glyphs. Only X axis is supported for now.
	GlyphOffset:          Vector2,   // 0, 0     // Offset all glyphs from this font input.
...
FontAtlasCustomRect :: struct {
	...
	GlyphOffset:   Vector2,     // Input    // For custom font glyphs only: glyph display offset
...
FontAtlas :: struct {
	...
	TexUvScale:         Vector2,                       // = (1.0f/TexWidth, 1.0f/TexHeight)
	TexUvWhitePixel:    Vector2,                       // Texture coordinates to a white pixel
...
Viewport :: struct {
	...
	Pos:      Vector2,          // Main Area: Position of the viewport (Dear ImGui coordinates are the same as OS desktop/native coordinates)
	Size:     Vector2,          // Main Area: Size of the viewport.
	WorkPos:  Vector2,          // Work Area: Position of the viewport minus task bars, menus bars, status bars (>= Pos)
	WorkSize: Vector2,          // Work Area: Size of the viewport minus task bars, menu bars, status bars (<= Size)
...
PlatformImeData :: struct {
	...
	InputPos:        Vector2, // Position of the input cursor
...
foreign lib {
	...
	// - Manual sizing (each axis can use a different setting e.g. ImVector2(0.0f, 400.0f)):
	...
	//    such as BeginMenu/EndMenu, BeginPopup/EndPopup, etc. where the EndXXX call should only be called if the corresponding
	//    BeginXXX function returned true. Begin and BeginChild are the only odd ones out. Will be fixed in a future update.]
	@(link_name="ImGui_BeginChild")   BeginChild   :: proc(str_id: cstring, size: Vector2 = {0, 0}, child_flags: ChildFlags = {}, window_flags: WindowFlags = {}) -> bool ---
	@(link_name="ImGui_BeginChildID") BeginChildID :: proc(id: ID, size: Vector2 = {0, 0}, child_flags: ChildFlags = {}, window_flags: WindowFlags = {}) -> bool          ---
	...
	@(link_name="ImGui_GetWindowPos")                 GetWindowPos                 :: proc() -> Vector2                                                                                                --- // get current window position in screen space (note: it is unlikely you need to use this. Consider using current layout pos instead, GetCursorScreenPos())
	@(link_name="ImGui_GetWindowSize")                GetWindowSize                :: proc() -> Vector2                                                                                                --- // get current window size (note: it is unlikely you need to use this. Consider using GetCursorScreenPos() and e.g. GetContentRegionAvail() instead)
	...
	@(link_name="ImGui_SetNextWindowPosEx")           SetNextWindowPos             :: proc(pos: Vector2, cond: Cond = {}, pivot: Vector2 = {0, 0})                                                        --- // set next window position. call before Begin(). use pivot=(0.5f,0.5f) to center on given point, etc.
	@(link_name="ImGui_SetNextWindowSize")            SetNextWindowSize            :: proc(size: Vector2, cond: Cond = {})                                                                             --- // set next window size. set axis to 0.0f to force an auto-fit on this axis. call before Begin()
	@(link_name="ImGui_SetNextWindowSizeConstraints") SetNextWindowSizeConstraints :: proc(size_min: Vector2, size_max: Vector2, custom_callback: SizeCallback = nil, custom_callback_data: rawptr = nil) --- // set next window size limits. use 0.0f or FLT_MAX if you don't want limits. Use -1 for both min and max of same axis to preserve current size (which itself is a constraint). Use callback to apply non-trivial programmatic constraints.
	@(link_name="ImGui_SetNextWindowContentSize")     SetNextWindowContentSize     :: proc(size: Vector2)                                                                                              --- // set next window content size (~ scrollable client area, which enforce the range of scrollbars). Not including window decorations (title bar, menu bar, etc.) nor WindowPadding. set an axis to 0.0f to leave it automatic. call before Begin()
	...
	@(link_name="ImGui_SetNextWindowScroll")          SetNextWindowScroll          :: proc(scroll: Vector2)                                                                                            --- // set next window scrolling value (use < 0.0f to not affect a given axis).
	...
	@(link_name="ImGui_SetWindowPos")                 SetWindowPos                 :: proc(pos: Vector2, cond: Cond = {})                                                                              --- // (not recommended) set current window position - call within Begin()/End(). prefer using SetNextWindowPos(), as this may incur tearing and side-effects.
	@(link_name="ImGui_SetWindowSize")                SetWindowSize                :: proc(size: Vector2, cond: Cond = {})                                                                             --- // (not recommended) set current window size - call within Begin()/End(). set to ImVector2(0, 0) to force an auto-fit. prefer using SetNextWindowSize(), as this may incur tearing and minor side-effects.
	...
	@(link_name="ImGui_SetWindowPosStr")              SetWindowPosStr              :: proc(name: cstring, pos: Vector2, cond: Cond = {})                                                               --- // set named window position.
	@(link_name="ImGui_SetWindowSizeStr")             SetWindowSizeStr             :: proc(name: cstring, size: Vector2, cond: Cond = {})                                                              --- // set named window size. set axis to 0.0f to force an auto-fit on this axis.
	...
	@(link_name="ImGui_GetContentRegionAvail")     GetContentRegionAvail     :: proc() -> Vector2 --- // == GetContentRegionMax() - GetCursorPos()
	@(link_name="ImGui_GetContentRegionMax")       GetContentRegionMax       :: proc() -> Vector2 --- // current content boundaries (typically window boundaries including scrolling, or current column boundaries), in windows coordinates
	@(link_name="ImGui_GetWindowContentRegionMin") GetWindowContentRegionMin :: proc() -> Vector2 --- // content boundaries min for the full window (roughly (0,0)-Scroll), in window coordinates
	@(link_name="ImGui_GetWindowContentRegionMax") GetWindowContentRegionMax :: proc() -> Vector2 --- // content boundaries max for the full window (roughly (0,0)+Size-Scroll) where Size can be overridden with SetNextWindowContentSize(), in window coordinates
	// Windows Scrolling
	...
	@(link_name="ImGui_PushStyleVarImVec2")   PushStyleVarImVec2   :: proc(idx: StyleVar, val: Vector2) --- // modify a style ImVector2 variable. always use this if you modify the style after NewFrame().
	...
	@(link_name="ImGui_GetFontTexUvWhitePixel") GetFontTexUvWhitePixel :: proc() -> Vector2                              --- // get UV coordinate for a while pixel, useful to draw custom shapes via the ImDrawList API
	...
	@(link_name="ImGui_GetCursorScreenPos") GetCursorScreenPos :: proc() -> Vector2        --- // cursor position in absolute coordinates (prefer using this, also more useful to work with ImDrawList API).
	@(link_name="ImGui_SetCursorScreenPos") SetCursorScreenPos :: proc(pos: Vector2)       --- // cursor position in absolute coordinates
	@(link_name="ImGui_GetCursorPos")       GetCursorPos       :: proc() -> Vector2        --- // [window-local] cursor position in window coordinates (relative to window position)
	...
	@(link_name="ImGui_SetCursorPos")       SetCursorPos       :: proc(local_pos: Vector2) --- // [window-local] "
	...
	@(link_name="ImGui_GetCursorStartPos")  GetCursorStartPos  :: proc() -> Vector2        --- // [window-local] initial cursor position, in window coordinates
	...
	@(link_name="ImGui_Dummy")                        Dummy                        :: proc(size: Vector2)                                          --- // add a dummy item of given size. unlike InvisibleButton(), Dummy() won't take the mouse click or be navigable into.
	...
	@(link_name="ImGui_ButtonEx")             Button               :: proc(label: cstring, size: Vector2 = {0, 0}) -> bool                                                                                                                               --- // button
	@(link_name="ImGui_SmallButton")          SmallButton          :: proc(label: cstring) -> bool                                                                                                                                                    --- // button with (FramePadding.y == 0) to easily embed within text
	@(link_name="ImGui_InvisibleButton")      InvisibleButton      :: proc(str_id: cstring, size: Vector2, flags: ButtonFlags = {}) -> bool                                                                                                              --- // flexible button behavior without the visuals, frequently useful to build custom behaviors using the public api (along with IsItemActive, IsItemHovered, etc.)
	...
	@(link_name="ImGui_ProgressBar")          ProgressBar          :: proc(fraction: f32, size_arg: Vector2 = {-min(f32), 0}, overlay: cstring = nil)                                                                                                    ---
	...
	@(link_name="ImGui_ImageEx")              Image                :: proc(user_texture_id: TextureID, image_size: Vector2, uv0: Vector2 = {0, 0}, uv1: Vector2 = {1, 1}, tint_col: Vec4 = {1, 1, 1, 1}, border_col: Vec4 = {0, 0, 0, 0})                      ---
	@(link_name="ImGui_ImageButtonEx")        ImageButton          :: proc(str_id: cstring, user_texture_id: TextureID, image_size: Vector2, uv0: Vector2 = {0, 0}, uv1: Vector2 = {1, 1}, bg_col: Vec4 = {0, 0, 0, 0}, tint_col: Vec4 = {1, 1, 1, 1}) -> bool ---
	...
	@(link_name="ImGui_VSliderFloatEx")       VSliderFloat       :: proc(label: cstring, size: Vector2, v: ^f32, v_min: f32, v_max: f32, format: cstring = "%.3f", flags: SliderFlags = {}) -> bool                                                                                     ---
	@(link_name="ImGui_VSliderIntEx")         VSliderInt         :: proc(label: cstring, size: Vector2, v: ^c.int, v_min: c.int, v_max: c.int, format: cstring = "%d", flags: SliderFlags = {}) -> bool                                                                                 ---
	@(link_name="ImGui_VSliderScalarEx")      VSliderScalar      :: proc(label: cstring, size: Vector2, data_type: DataType, p_data: rawptr, p_min: rawptr, p_max: rawptr, format: cstring = nil, flags: SliderFlags = {}) -> bool                                                      ---
	...
	@(link_name="ImGui_InputTextMultilineEx") InputTextMultiline :: proc(label: cstring, buf: cstring, buf_size: c.size_t, size: Vector2 = {0, 0}, flags: InputTextFlags = {}, callback: InputTextCallback = nil, user_data: rawptr = nil) -> bool                                      ---
	...
	@(link_name="ImGui_ColorButtonEx")       ColorButton         :: proc(desc_id: cstring, col: Vec4, flags: ColorEditFlags = {}, size: Vector2 = {0, 0}) -> bool  --- // display a color square/button, hover for details, return true when pressed.
	...
	@(link_name="ImGui_SelectableEx")              Selectable                :: proc(label: cstring, selected: bool = false, flags: SelectableFlags = {}, size: Vector2 = {0, 0}) -> bool --- // "bool selected" carry the selection state (read-only). Selectable() is clicked is returns true so you can modify your selection state. size.x==0.0: use remaining width, size.x>0.0: specify width. size.y==0.0: use label height, size.y>0.0: specify height
	@(link_name="ImGui_SelectableBoolPtrEx")       SelectableBoolPtr         :: proc(label: cstring, p_selected: ^bool, flags: SelectableFlags = {}, size: Vector2 = {0, 0}) -> bool      --- // "bool* p_selected" point to the selection state (read-write), as a convenient helper.
	// Widgets: List Boxes
	...
	@(link_name="ImGui_BeginListBox")            BeginListBox          :: proc(label: cstring, size: Vector2 = {0, 0}) -> bool                                                                                                                                                                                                           --- // open a framed scrolling region
	...
	@(link_name="ImGui_PlotLinesEx")             PlotLines             :: proc(label: cstring, values: ^f32, values_count: c.int, values_offset: c.int = {}, overlay_text: cstring = nil, scale_min: f32 = max(f32), scale_max: f32 = max(f32), graph_size: Vector2 = {0, 0}, stride: c.int = size_of(f32))                              ---
	@(link_name="ImGui_PlotLinesCallbackEx")     PlotLinesCallback     :: proc(label: cstring, values_getter: proc "c" (data: rawptr, idx: c.int) -> f32, data: rawptr, values_count: c.int, values_offset: c.int = {}, overlay_text: cstring = nil, scale_min: f32 = max(f32), scale_max: f32 = max(f32), graph_size: Vector2 = {0, 0}) ---
	@(link_name="ImGui_PlotHistogramEx")         PlotHistogram         :: proc(label: cstring, values: ^f32, values_count: c.int, values_offset: c.int = {}, overlay_text: cstring = nil, scale_min: f32 = max(f32), scale_max: f32 = max(f32), graph_size: Vector2 = {0, 0}, stride: c.int = size_of(f32))                              ---
	@(link_name="ImGui_PlotHistogramCallbackEx") PlotHistogramCallback :: proc(label: cstring, values_getter: proc "c" (data: rawptr, idx: c.int) -> f32, data: rawptr, values_count: c.int, values_offset: c.int = {}, overlay_text: cstring = nil, scale_min: f32 = max(f32), scale_max: f32 = max(f32), graph_size: Vector2 = {0, 0}) ---
	// Widgets: Menus
	...
	@(link_name="ImGui_BeginTableEx")           BeginTable             :: proc(str_id: cstring, column: c.int, flags: TableFlags = {}, outer_size: Vector2 = {0.0, 0.0}, inner_width: f32 = 0.0) -> bool ---
	...
	@(link_name="ImGui_PushClipRect") PushClipRect :: proc(clip_rect_min: Vector2, clip_rect_max: Vector2, intersect_with_current_clip_rect: bool) ---
	...
	@(link_name="ImGui_GetItemRectMin")             GetItemRectMin             :: proc() -> Vector2                               --- // get upper-left bounding rectangle of the last item (screen space)
	@(link_name="ImGui_GetItemRectMax")             GetItemRectMax             :: proc() -> Vector2                               --- // get lower-right bounding rectangle of the last item (screen space)
	@(link_name="ImGui_GetItemRectSize")            GetItemRectSize            :: proc() -> Vector2                               --- // get size of last item
	// Viewports
	// - Currently represents the Platform Window created by the application which is hosting our Dear ImGui windows.
	// - In 'docking' branch with multi-viewport enabled, we extend this concept to have multiple active viewports.
	// - In the future we will extend this concept further to also represent Platform Monitor and support a "no main platform window" operation mode.
	...
	@(link_name="ImGui_IsRectVisibleBySize")   IsRectVisibleBySize   :: proc(size: Vector2) -> bool                                                                                                --- // test if rectangle (of given size, starting from cursor position) is visible / not clipped.
	@(link_name="ImGui_IsRectVisible")         IsRectVisible         :: proc(rect_min: Vector2, rect_max: Vector2) -> bool                                                                            --- // test if rectangle (in screen space) is visible / not clipped. to perform coarse clipping on user's side.
	...
	@(link_name="ImGui_CalcTextSizeEx")        CalcTextSize          :: proc(text: cstring, text_end: cstring = nil, hide_text_after_double_hash: bool = false, wrap_width: f32 = -1.0) -> Vector2 ---
	// Color Utilities
	...
	@(link_name="ImGui_IsMouseHoveringRectEx")            IsMouseHoveringRect              :: proc(r_min: Vector2, r_max: Vector2, clip: bool = true) -> bool          --- // is mouse hovering given bounding rect (in screen space). clipped by current clipping settings, but disregarding of other consideration of focus/window ordering/popup-block.
	@(link_name="ImGui_IsMousePosValid")                  IsMousePosValid                  :: proc(mouse_pos: ^Vector2 = nil) -> bool                               --- // by convention we use (-FLT_MAX,-FLT_MAX) to denote that there is no mouse available
	...
	@(link_name="ImGui_GetMousePos")                      GetMousePos                      :: proc() -> Vector2                                                     --- // shortcut to ImGui::GetIO().MousePos provided by user, to be consistent with other calls
	@(link_name="ImGui_GetMousePosOnOpeningCurrentPopup") GetMousePosOnOpeningCurrentPopup :: proc() -> Vector2                                                     --- // retrieve mouse position at the time of opening popup we have BeginPopup() into (helper to avoid user backing that value themselves)
	...
	@(link_name="ImGui_GetMouseDragDelta")                GetMouseDragDelta                :: proc(button: MouseButton = {}, lock_threshold: f32 = -1.0) -> Vector2 --- // return the delta from the initial clicking position while the mouse button is pressed or was just released. This is locked and return 0.0f until the mouse moves past a distance threshold at least once (if lock_threshold < -1.0f, uses io.MouseDraggingThreshold)
	...
	@(link_name="ImDrawList_GetClipRectMin")            DrawList_GetClipRectMin            :: proc(self: ^DrawList) -> Vector2                                                                                                                                                                       ---
	@(link_name="ImDrawList_GetClipRectMax")            DrawList_GetClipRectMax            :: proc(self: ^DrawList) -> Vector2                                                                                                                                                                       ---
	@(link_name="ImDrawList_AddLineEx")                 DrawList_AddLine                   :: proc(self: ^DrawList, p1: Vector2, p2: Vector2, col: u32, thickness: f32 = 1.0)                                                                                                                           ---
	@(link_name="ImDrawList_AddRectEx")                 DrawList_AddRect                   :: proc(self: ^DrawList, p_min: Vector2, p_max: Vector2, col: u32, rounding: f32 = 0.0, flags: DrawFlags = {}, thickness: f32 = 1.0)                                                                         --- // a: upper-left, b: lower-right (== upper-left + size)
	@(link_name="ImDrawList_AddRectFilledEx")           DrawList_AddRectFilled             :: proc(self: ^DrawList, p_min: Vector2, p_max: Vector2, col: u32, rounding: f32 = 0.0, flags: DrawFlags = {})                                                                                               --- // a: upper-left, b: lower-right (== upper-left + size)
	@(link_name="ImDrawList_AddRectFilledMultiColor")   DrawList_AddRectFilledMultiColor   :: proc(self: ^DrawList, p_min: Vector2, p_max: Vector2, col_upr_left: u32, col_upr_right: u32, col_bot_right: u32, col_bot_left: u32)                                                                       ---
	@(link_name="ImDrawList_AddQuadEx")                 DrawList_AddQuad                   :: proc(self: ^DrawList, p1: Vector2, p2: Vector2, p3: Vector2, p4: Vector2, col: u32, thickness: f32 = 1.0)                                                                                                       ---
	@(link_name="ImDrawList_AddQuadFilled")             DrawList_AddQuadFilled             :: proc(self: ^DrawList, p1: Vector2, p2: Vector2, p3: Vector2, p4: Vector2, col: u32)                                                                                                                             ---
	@(link_name="ImDrawList_AddTriangleEx")             DrawList_AddTriangle               :: proc(self: ^DrawList, p1: Vector2, p2: Vector2, p3: Vector2, col: u32, thickness: f32 = 1.0)                                                                                                                 ---
	@(link_name="ImDrawList_AddTriangleFilled")         DrawList_AddTriangleFilled         :: proc(self: ^DrawList, p1: Vector2, p2: Vector2, p3: Vector2, col: u32)                                                                                                                                       ---
	@(link_name="ImDrawList_AddCircleEx")               DrawList_AddCircle                 :: proc(self: ^DrawList, center: Vector2, radius: f32, col: u32, num_segments: c.int = {}, thickness: f32 = 1.0)                                                                                          ---
	@(link_name="ImDrawList_AddCircleFilled")           DrawList_AddCircleFilled           :: proc(self: ^DrawList, center: Vector2, radius: f32, col: u32, num_segments: c.int = {})                                                                                                                ---
	@(link_name="ImDrawList_AddNgonEx")                 DrawList_AddNgon                   :: proc(self: ^DrawList, center: Vector2, radius: f32, col: u32, num_segments: c.int, thickness: f32 = 1.0)                                                                                               ---
	@(link_name="ImDrawList_AddNgonFilled")             DrawList_AddNgonFilled             :: proc(self: ^DrawList, center: Vector2, radius: f32, col: u32, num_segments: c.int)                                                                                                                     ---
	@(link_name="ImDrawList_AddEllipseEx")              DrawList_AddEllipse                :: proc(self: ^DrawList, center: Vector2, radius_x: f32, radius_y: f32, col: u32, rot: f32 = 0.0, num_segments: c.int = {}, thickness: f32 = 1.0)                                                         ---
	@(link_name="ImDrawList_AddEllipseFilledEx")        DrawList_AddEllipseFilled          :: proc(self: ^DrawList, center: Vector2, radius_x: f32, radius_y: f32, col: u32, rot: f32 = 0.0, num_segments: c.int = {})                                                                               ---
	@(link_name="ImDrawList_AddTextEx")                 DrawList_AddText                   :: proc(self: ^DrawList, pos: Vector2, col: u32, text_begin: cstring, text_end: cstring = nil)                                                                                                            ---
	@(link_name="ImDrawList_AddTextImFontPtrEx")        DrawList_AddTextImFontPtr          :: proc(self: ^DrawList, font: ^Font, font_size: f32, pos: Vector2, col: u32, text_begin: cstring, text_end: cstring = nil, wrap_width: f32 = 0.0, cpu_fine_clip_rect: ^Vec4 = nil)                       ---
	@(link_name="ImDrawList_AddPolyline")               DrawList_AddPolyline               :: proc(self: ^DrawList, points: ^Vector2, num_points: c.int, col: u32, flags: DrawFlags, thickness: f32)                                                                                                 ---
	@(link_name="ImDrawList_AddConvexPolyFilled")       DrawList_AddConvexPolyFilled       :: proc(self: ^DrawList, points: ^Vector2, num_points: c.int, col: u32)                                                                                                                                   ---
	@(link_name="ImDrawList_AddBezierCubic")            DrawList_AddBezierCubic            :: proc(self: ^DrawList, p1: Vector2, p2: Vector2, p3: Vector2, p4: Vector2, col: u32, thickness: f32, num_segments: c.int = {})                                                                                   --- // Cubic Bezier (4 control points)
	@(link_name="ImDrawList_AddBezierQuadratic")        DrawList_AddBezierQuadratic        :: proc(self: ^DrawList, p1: Vector2, p2: Vector2, p3: Vector2, col: u32, thickness: f32, num_segments: c.int = {})                                                                                             --- // Quadratic Bezier (3 control points)
	@(link_name="ImDrawList_AddImageEx")                DrawList_AddImage                  :: proc(self: ^DrawList, user_texture_id: TextureID, p_min: Vector2, p_max: Vector2, uv_min: Vector2 = {0, 0}, uv_max: Vector2 = {1, 1}, col: u32 = 0xff_ff_ff_ff)                                                 ---
	@(link_name="ImDrawList_AddImageQuadEx")            DrawList_AddImageQuad              :: proc(self: ^DrawList, user_texture_id: TextureID, p1: Vector2, p2: Vector2, p3: Vector2, p4: Vector2, uv1: Vector2 = {0, 0}, uv2: Vector2 = {1, 0}, uv3: Vector2 = {1, 1}, uv4: Vector2 = {0, 1}, col: u32 = 0xff_ff_ff_ff) ---
	@(link_name="ImDrawList_AddImageRounded")           DrawList_AddImageRounded           :: proc(self: ^DrawList, user_texture_id: TextureID, p_min: Vector2, p_max: Vector2, uv_min: Vector2, uv_max: Vector2, col: u32, rounding: f32, flags: DrawFlags = {})                                             ---
	// Stateful path API, add points then finish with PathFillConvex() or PathStroke()
	// - Filled shapes must always use clockwise winding order. The anti-aliasing fringe depends on it. Counter-clockwise shapes will have "inward" anti-aliasing.
	@(link_name="ImDrawList_PathClear")                  DrawList_PathClear                  :: proc(self: ^DrawList)                                                                                                         ---
	@(link_name="ImDrawList_PathLineTo")                 DrawList_PathLineTo                 :: proc(self: ^DrawList, pos: Vector2)                                                                                              ---
	@(link_name="ImDrawList_PathLineToMergeDuplicate")   DrawList_PathLineToMergeDuplicate   :: proc(self: ^DrawList, pos: Vector2)                                                                                              ---
	...
	@(link_name="ImDrawList_PathArcTo")                  DrawList_PathArcTo                  :: proc(self: ^DrawList, center: Vector2, radius: f32, a_min: f32, a_max: f32, num_segments: c.int = {})                            ---
	@(link_name="ImDrawList_PathArcToFast")              DrawList_PathArcToFast              :: proc(self: ^DrawList, center: Vector2, radius: f32, a_min_of_12: c.int, a_max_of_12: c.int)                                      --- // Use precomputed angles for a 12 steps circle
	@(link_name="ImDrawList_PathEllipticalArcToEx")      DrawList_PathEllipticalArcTo        :: proc(self: ^DrawList, center: Vector2, radius_x: f32, radius_y: f32, rot: f32, a_min: f32, a_max: f32, num_segments: c.int = {}) --- // Ellipse
	@(link_name="ImDrawList_PathBezierCubicCurveTo")     DrawList_PathBezierCubicCurveTo     :: proc(self: ^DrawList, p2: Vector2, p3: Vector2, p4: Vector2, num_segments: c.int = {})                                                 --- // Cubic Bezier (4 control points)
	@(link_name="ImDrawList_PathBezierQuadraticCurveTo") DrawList_PathBezierQuadraticCurveTo :: proc(self: ^DrawList, p2: Vector2, p3: Vector2, num_segments: c.int = {})                                                           --- // Quadratic Bezier (3 control points)
	@(link_name="ImDrawList_PathRect")                   DrawList_PathRect                   :: proc(self: ^DrawList, rect_min: Vector2, rect_max: Vector2, rounding: f32 = 0.0, flags: DrawFlags = {})                             ---
	// Advanced
	...
	@(link_name="ImDrawList_PrimRect")      DrawList_PrimRect      :: proc(self: ^DrawList, a: Vector2, b: Vector2, col: u32)                                                                    --- // Axis aligned rectangle (composed of two triangles)
	@(link_name="ImDrawList_PrimRectUV")    DrawList_PrimRectUV    :: proc(self: ^DrawList, a: Vector2, b: Vector2, uv_a: Vector2, uv_b: Vector2, col: u32)                                            ---
	@(link_name="ImDrawList_PrimQuadUV")    DrawList_PrimQuadUV    :: proc(self: ^DrawList, a: Vector2, b: Vector2, _c: Vector2, d: Vector2, uv_a: Vector2, uv_b: Vector2, uv_c: Vector2, uv_d: Vector2, col: u32) ---
	@(link_name="ImDrawList_PrimWriteVtx")  DrawList_PrimWriteVtx  :: proc(self: ^DrawList, pos: Vector2, uv: Vector2, col: u32)                                                                 ---
	@(link_name="ImDrawList_PrimWriteIdx")  DrawList_PrimWriteIdx  :: proc(self: ^DrawList, idx: DrawIdx)                                                                                  ---
	@(link_name="ImDrawList_PrimVtx")       DrawList_PrimVtx       :: proc(self: ^DrawList, pos: Vector2, uv: Vector2, col: u32)                                                                 --- // Write vertex with unique index
	// [Internal helpers]
	...
	@(link_name="ImDrawList__PathArcToFastEx")                      DrawList__PathArcToFastEx                      :: proc(self: ^DrawList, center: Vector2, radius: f32, a_min_sample: c.int, a_max_sample: c.int, a_step: c.int)                                                                  ---
	@(link_name="ImDrawList__PathArcToN")                           DrawList__PathArcToN                           :: proc(self: ^DrawList, center: Vector2, radius: f32, a_min: f32, a_max: f32, num_segments: c.int)                                                                              ---
	...
	@(link_name="ImDrawData_ScaleClipRects")                        DrawData_ScaleClipRects                        :: proc(self: ^DrawData, fb_scale: Vector2)                                                                                                                                      --- // Helper to scale the ClipRect field of each ImDrawCmd. Use if your final output buffer is at a different scale than Dear ImGui expects, or if there is a difference between your window resolution and framebuffer resolution.
	...
	@(link_name="ImFontAtlas_AddCustomRectFontGlyph") FontAtlas_AddCustomRectFontGlyph :: proc(self: ^FontAtlas, font: ^Font, id: Wchar, width: c.int, height: c.int, advance_x: f32, offset: Vector2 = {0, 0}) -> c.int ---
	...
	// [Internal]
	@(link_name="ImFontAtlas_CalcCustomRectUV")      FontAtlas_CalcCustomRectUV      :: proc(self: ^FontAtlas, rect: ^FontAtlasCustomRect, out_uv_min: ^Vector2, out_uv_max: ^Vector2)                                                                                             ---
	@(link_name="ImFontAtlas_GetMouseCursorTexData") FontAtlas_GetMouseCursorTexData :: proc(self: ^FontAtlas, cursor: MouseCursor, out_offset: ^Vector2, out_size: ^Vector2, out_uv_border: ^[2]Vector2, out_uv_fill: ^[2]Vector2) -> bool                                              ---
	...
	@(link_name="ImFont_CalcTextSizeAEx")            Font_CalcTextSizeA              :: proc(self: ^Font, size: f32, max_width: f32, wrap_width: f32, text_begin: cstring, text_end: cstring = nil, remaining: ^cstring = nil) -> Vector2                                       --- // utf8
	...
	@(link_name="ImFont_RenderChar")                 Font_RenderChar                 :: proc(self: ^Font, draw_list: ^DrawList, size: f32, pos: Vector2, col: u32, _c: Wchar)                                                                                                   ---
	@(link_name="ImFont_RenderText")                 Font_RenderText                 :: proc(self: ^Font, draw_list: ^DrawList, size: f32, pos: Vector2, col: u32, clip_rect: Vec4, text_begin: cstring, text_end: cstring, wrap_width: f32 = 0.0, cpu_fine_clip: bool = false) ---
	// [Internal] Don't use!
	...
	// Helpers
	@(link_name="ImGuiViewport_GetCenter")     Viewport_GetCenter     :: proc(self: ^Viewport) -> Vector2                                    ---
	@(link_name="ImGuiViewport_GetWorkCenter") Viewport_GetWorkCenter :: proc(self: ^Viewport) -> Vector2                                    ---
	@(link_name="ImGui_GetKeyIndex")           GetKeyIndex            :: proc(key: Key) -> Key                                            --- // map ImGuiKey_* values into legacy native key index. == io.KeyMap[key]
	@(link_name="ImGui_BeginChildFrameEx")     BeginChildFrame        :: proc(id: ID, size: Vector2, window_flags: WindowFlags = {}) -> bool ---
	@(link_name="ImGui_EndChildFrame")         EndChildFrame          :: proc()                                                           ---
	//static inline bool BeginChild(const char* str_id, const ImVector2& size_arg, bool border, ImGuiWindowFlags window_flags){ return BeginChild(str_id, size_arg, border ? ImGuiChildFlags_Border : ImGuiChildFlags_None, window_flags); } // Unnecessary as true == ImGuiChildFlags_Border
	//static inline bool BeginChild(ImGuiID id, const ImVector2& size_arg, bool border, ImGuiWindowFlags window_flags)        { return BeginChild(id, size_arg, border ? ImGuiChildFlags_Border : ImGuiChildFlags_None, window_flags);     } // Unnecessary as true == ImGuiChildFlags_Border
	...
	@(link_name="ImGui_ImageButtonImTextureID") ImageButtonImTextureID :: proc(user_texture_id: TextureID, size: Vector2, uv0: Vector2 = {0, 0}, uv1: Vector2 = {1, 1}, frame_padding: c.int = -1, bg_col: Vec4 = {0, 0, 0, 0}, tint_col: Vec4 = {1, 1, 1, 1}) -> bool --- // Use new ImageButton() signature (explicit item id, regular FramePadding)

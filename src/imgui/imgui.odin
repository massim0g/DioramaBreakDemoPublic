...
when ODIN_OS == .Darwin {
	foreign import lib { IMGUI_LIB_NAME, "system:c++" }
} else when ODIN_OS == .Linux {
	//the .a is built against libstdc++ (guaranteed by the Steam runtime), not libc++
	foreign import lib { IMGUI_LIB_NAME, "system:stdc++" }
} else when ODIN_OS == .JS {
	foreign import lib "wasm/dcimgui_nodefaultargfunctions.o"
	...
	foreign import lib { IMGUI_LIB_NAME }
}
...
PopupFlags :: distinct i32
PopupFlags_None              :: PopupFlags({})
...
HoveredFlags :: distinct i32
HoveredFlags_None                    :: HoveredFlags({})   // Return true if directly over the item/window, not obstructed by another window, not obstructed by an active popup or modal blocking inputs under them.
...
Dir :: enum i32 { // Forward declared enum type ImGuiDir
	None = -1,
	Left,
	Right,
	Up,
...
// A sorting direction
SortDirection :: enum i32 { // Forward declared enum type ImGuiSortDirection
	None,
...
Key :: enum i32 { // Forward declared enum type ImGuiKey
	// Keyboard
	None,
...
SliderFlags_InvalidMask_ :: i32(0x7000000F) // Meant to be of type SliderFlags // [Internal] We treat using those bits as being potentially a 'float power' argument from legacy API (obsoleted 2020-08) that has got miscast to this enum, and will trigger an assert if needed.
...
MouseSource :: enum i32 { // Forward declared enum type ImGuiMouseSource
...
TableFlags :: distinct i32
// Features
TableFlags_None              :: TableFlags({})
...
DrawFlags :: distinct i32
DrawFlags_None                    :: DrawFlags({})
...
DrawFlags_Closed                  :: DrawFlags(1<<9)                                                                                                                                  // PathStroke(), AddPolyline(): specify that shape should be closed (Important: this is always == 1 for legacy reason)
...
DrawFlags_InvalidMask_            :: DrawFlags(-2147483633)
...
SelectionBasicStorage :: struct {
	...
	AdapterIndexToStorageId: proc "c" (self: ^SelectionBasicStorage, idx: i32) -> ID, // e.g. selection.AdapterIndexToStorageId = [](ImGuiSelectionBasicStorage* self, int idx) { return ((MyItems**)self->UserData)[idx]->ID; };
...
SelectionExternalStorage :: struct {
	...
	AdapterSetItemSelected: proc "c" (self: ^SelectionExternalStorage, idx: i32, selected: bool), // e.g. AdapterSetItemSelected = [](ImGuiSelectionExternalStorage* self, int idx, bool selected) { ((MyItems**)self->UserData)[idx]->Selected = selected; }
}
...
PlatformIO :: struct {
	...
	Platform_CreateVkSurface:           proc "c" (vp: ^Viewport, vk_inst: u64, vk_allocators: rawptr, out_vk_surface: ^u64) -> i32, // (Optional) For a Vulkan Renderer to call into Platform code (since the surface creation needs to tie them both).
...
foreign lib {
	...
	@(link_name="ImGui_PopStyleColor")        PopStyleColor        :: proc(count: i32 = 1)                   ---
	...
	@(link_name="ImGui_PopStyleVar")          PopStyleVar          :: proc(count: i32 = 1)                   ---
	...
	@(link_name="ImGui_PushIDInt") PushIDInt :: proc(int_id: i32)                                      --- // push integer into the ID stack (will hash integer).
	...
	@(link_name="ImGui_GetIDInt")  GetIDInt  :: proc(int_id: i32) -> ID                                ---
	// Widgets: Text
	...
	@(link_name="ImGui_TextColoredUnformatted")  TextColoredUnformatted  :: proc(col: Vec4, text: cstring)                            --- // shortcut for PushStyleColor(ImGuiCol_Text, col); Text(fmt, ...); PopStyleColor();
	...
	@(link_name="ImGui_TextDisabledUnformatted") TextDisabledUnformatted :: proc(text: cstring)                                       --- // shortcut for PushStyleColor(ImGuiCol_Text, style.Colors[ImGuiCol_TextDisabled]); Text(fmt, ...); PopStyleColor();
	...
	@(link_name="ImGui_TextWrappedUnformatted")  TextWrappedUnformatted  :: proc(text: cstring)                                       --- // shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note that this won't work on an auto-resizing window if there's no other widgets to extend the window width, yoy may need to set a size using SetNextWindowSize().
	...
	@(link_name="ImGui_LabelTextUnformatted")    LabelTextUnformatted    :: proc(label: cstring, text: cstring)                       --- // display text+label aligned the same way as value+label widgets
	@(link_name="ImGui_BulletText")              BulletText              :: proc(fmt: cstring, #c_vararg args: ..any)                 --- // shortcut for Bullet()+Text()
	@(link_name="ImGui_BulletTextUnformatted")   BulletTextUnformatted   :: proc(text: cstring)                                       --- // shortcut for Bullet()+Text()
	...
	@(link_name="ImGui_CheckboxFlagsIntPtr")  CheckboxFlagsIntPtr  :: proc(label: cstring, flags: ^i32, flags_value: i32) -> bool                      ---
	@(link_name="ImGui_CheckboxFlagsUintPtr") CheckboxFlagsUintPtr :: proc(label: cstring, flags: ^u32, flags_value: u32) -> bool                      ---
	...
	@(link_name="ImGui_RadioButtonIntPtr")    RadioButtonIntPtr    :: proc(label: cstring, v: ^i32, v_button: i32) -> bool                             --- // shortcut to handle the above pattern when value is an integer
	@(link_name="ImGui_ProgressBar")          ProgressBar          :: proc(fraction: f32, size_arg: Vec2 = {-math.F32_MIN, 0}, overlay: cstring = nil) ---
	...
	@(link_name="ImGui_ComboChar")     ComboChar     :: proc(label: cstring, current_item: ^i32, items: [^]cstring, items_count: i32, popup_max_height_in_items: i32 = -1) -> bool                                                            ---
	@(link_name="ImGui_Combo")         Combo         :: proc(label: cstring, current_item: ^i32, items_separated_by_zeros: cstring, popup_max_height_in_items: i32 = -1) -> bool                                                              --- // Separate items with \0 within a string, end item-list with \0\0. e.g. "One\0Two\0Three\0"
	@(link_name="ImGui_ComboCallback") ComboCallback :: proc(label: cstring, current_item: ^i32, getter: proc "c" (user_data: rawptr, idx: i32) -> cstring, user_data: rawptr, items_count: i32, popup_max_height_in_items: i32 = -1) -> bool ---
	...
	@(link_name="ImGui_DragInt")         DragInt         :: proc(label: cstring, v: ^i32, v_speed: f32 = 1.0, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", flags: SliderFlags = {}) -> bool                                                                   --- // If v_min >= v_max we have no bound
	@(link_name="ImGui_DragInt2")        DragInt2        :: proc(label: cstring, v: ^[2]i32, v_speed: f32 = 1.0, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", flags: SliderFlags = {}) -> bool                                                                ---
	@(link_name="ImGui_DragInt3")        DragInt3        :: proc(label: cstring, v: ^[3]i32, v_speed: f32 = 1.0, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", flags: SliderFlags = {}) -> bool                                                                ---
	@(link_name="ImGui_DragInt4")        DragInt4        :: proc(label: cstring, v: ^[4]i32, v_speed: f32 = 1.0, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", flags: SliderFlags = {}) -> bool                                                                ---
	@(link_name="ImGui_DragIntRange2")   DragIntRange2   :: proc(label: cstring, v_current_min: ^i32, v_current_max: ^i32, v_speed: f32 = 1.0, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", format_max: cstring = nil, flags: SliderFlags = {}) -> bool       ---
	...
	@(link_name="ImGui_DragScalarN")     DragScalarN     :: proc(label: cstring, data_type: DataType, p_data: rawptr, components: i32, v_speed: f32 = 1.0, p_min: rawptr = nil, p_max: rawptr = nil, format: cstring = nil, flags: SliderFlags = {}) -> bool             ---
	...
	@(link_name="ImGui_SliderInt")     SliderInt     :: proc(label: cstring, v: ^i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: SliderFlags = {}) -> bool                                                   ---
	@(link_name="ImGui_SliderInt2")    SliderInt2    :: proc(label: cstring, v: ^[2]i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: SliderFlags = {}) -> bool                                                ---
	@(link_name="ImGui_SliderInt3")    SliderInt3    :: proc(label: cstring, v: ^[3]i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: SliderFlags = {}) -> bool                                                ---
	@(link_name="ImGui_SliderInt4")    SliderInt4    :: proc(label: cstring, v: ^[4]i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: SliderFlags = {}) -> bool                                                ---
	...
	@(link_name="ImGui_SliderScalarN") SliderScalarN :: proc(label: cstring, data_type: DataType, p_data: rawptr, components: i32, p_min: rawptr, p_max: rawptr, format: cstring = nil, flags: SliderFlags = {}) -> bool ---
	...
	@(link_name="ImGui_VSliderInt")    VSliderInt    :: proc(label: cstring, size: Vec2, v: ^i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: SliderFlags = {}) -> bool                                       ---
	...
	@(link_name="ImGui_InputText")          InputText          :: proc(label: cstring, buf: cstring, buf_size: uint, flags: InputTextFlags = {}, callback: InputTextCallback = nil, user_data: rawptr = nil) -> bool                                     ---
	@(link_name="ImGui_InputTextMultiline") InputTextMultiline :: proc(label: cstring, buf: cstring, buf_size: uint, size: Vec2 = {0, 0}, flags: InputTextFlags = {}, callback: InputTextCallback = nil, user_data: rawptr = nil) -> bool                ---
	@(link_name="ImGui_InputTextWithHint")  InputTextWithHint  :: proc(label: cstring, hint: cstring, buf: cstring, buf_size: uint, flags: InputTextFlags = {}, callback: InputTextCallback = nil, user_data: rawptr = nil) -> bool                      ---
	...
	@(link_name="ImGui_InputInt")           InputInt           :: proc(label: cstring, v: ^i32, step: i32 = 1, step_fast: i32 = 100, flags: InputTextFlags = {}) -> bool                                                                                 ---
	@(link_name="ImGui_InputInt2")          InputInt2          :: proc(label: cstring, v: ^[2]i32, flags: InputTextFlags = {}) -> bool                                                                                                                   ---
	@(link_name="ImGui_InputInt3")          InputInt3          :: proc(label: cstring, v: ^[3]i32, flags: InputTextFlags = {}) -> bool                                                                                                                   ---
	@(link_name="ImGui_InputInt4")          InputInt4          :: proc(label: cstring, v: ^[4]i32, flags: InputTextFlags = {}) -> bool                                                                                                                   ---
	...
	@(link_name="ImGui_InputScalarN")       InputScalarN       :: proc(label: cstring, data_type: DataType, p_data: rawptr, components: i32, p_step: rawptr = nil, p_step_fast: rawptr = nil, format: cstring = nil, flags: InputTextFlags = {}) -> bool ---
	...
	@(link_name="ImGui_TreeNodeStrUnformatted")    TreeNodeStrUnformatted    :: proc(str_id: cstring, text: cstring) -> bool                                             --- // helper variation to easily decorrelate the id from the displayed string. Read the FAQ about why and how to use ID. to align arbitrary text at the same level as a TreeNode() you can use Bullet().
	@(link_name="ImGui_TreeNodePtr")               TreeNodePtr               :: proc(ptr_id: rawptr, fmt: cstring, #c_vararg args: ..any) -> bool                        --- // "
	@(link_name="ImGui_TreeNodePtrUnformatted")    TreeNodePtrUnformatted    :: proc(ptr_id: rawptr, text: cstring) -> bool                                              --- // "
	...
	@(link_name="ImGui_TreeNodeExStrUnformatted")  TreeNodeExStrUnformatted  :: proc(str_id: cstring, flags: TreeNodeFlags, text: cstring) -> bool                       ---
	@(link_name="ImGui_TreeNodeExPtr")             TreeNodeExPtr             :: proc(ptr_id: rawptr, flags: TreeNodeFlags, fmt: cstring, #c_vararg args: ..any) -> bool  ---
	@(link_name="ImGui_TreeNodeExPtrUnformatted")  TreeNodeExPtrUnformatted  :: proc(ptr_id: rawptr, flags: TreeNodeFlags, text: cstring) -> bool                        ---
	...
	@(link_name="ImGui_BeginMultiSelect")             BeginMultiSelect             :: proc(flags: MultiSelectFlags, selection_size: i32 = -1, items_count: i32 = -1) -> ^MultiSelectIO ---
	...
	@(link_name="ImGui_ListBox")         ListBox         :: proc(label: cstring, current_item: ^i32, items: [^]cstring, items_count: i32, height_in_items: i32 = -1) -> bool                                                            ---
	@(link_name="ImGui_ListBoxCallback") ListBoxCallback :: proc(label: cstring, current_item: ^i32, getter: proc "c" (user_data: rawptr, idx: i32) -> cstring, user_data: rawptr, items_count: i32, height_in_items: i32 = -1) -> bool ---
	// Widgets: Data Plotting
	// - Consider using ImPlot (https://github.com/epezent/implot) which is much better!
	@(link_name="ImGui_PlotLines")             PlotLines             :: proc(label: cstring, values: ^f32, values_count: i32, values_offset: i32 = 0, overlay_text: cstring = nil, scale_min: f32 = math.F32_MAX, scale_max: f32 = math.F32_MAX, graph_size: Vec2 = {0, 0}, stride: i32 = size_of(f32))                              ---
	@(link_name="ImGui_PlotLinesCallback")     PlotLinesCallback     :: proc(label: cstring, values_getter: proc "c" (data: rawptr, idx: i32) -> f32, data: rawptr, values_count: i32, values_offset: i32 = 0, overlay_text: cstring = nil, scale_min: f32 = math.F32_MAX, scale_max: f32 = math.F32_MAX, graph_size: Vec2 = {0, 0}) ---
	@(link_name="ImGui_PlotHistogram")         PlotHistogram         :: proc(label: cstring, values: ^f32, values_count: i32, values_offset: i32 = 0, overlay_text: cstring = nil, scale_min: f32 = math.F32_MAX, scale_max: f32 = math.F32_MAX, graph_size: Vec2 = {0, 0}, stride: i32 = size_of(f32))                              ---
	@(link_name="ImGui_PlotHistogramCallback") PlotHistogramCallback :: proc(label: cstring, values_getter: proc "c" (data: rawptr, idx: i32) -> f32, data: rawptr, values_count: i32, values_offset: i32 = 0, overlay_text: cstring = nil, scale_min: f32 = math.F32_MAX, scale_max: f32 = math.F32_MAX, graph_size: Vec2 = {0, 0}) ---
	// Widgets: Menus
	...
	@(link_name="ImGui_SetTooltipUnformatted") SetTooltipUnformatted :: proc(text: cstring)                       --- // set a text-only tooltip. Often used after a ImGui::IsItemHovered() check. Override any previous call to SetTooltip().
	...
	@(link_name="ImGui_SetItemTooltipUnformatted") SetItemTooltipUnformatted :: proc(text: cstring)                       --- // set a text-only tooltip if preceding item was hovered. override any previous call to SetTooltip().
	// Popups, Modals
	...
	// - 5. Call EndTable()
	@(link_name="ImGui_BeginTable")          BeginTable          :: proc(str_id: cstring, columns: i32, flags: TableFlags = {}, outer_size: Vec2 = {0.0, 0.0}, inner_width: f32 = 0.0) -> bool ---
	...
	@(link_name="ImGui_TableSetColumnIndex") TableSetColumnIndex :: proc(column_n: i32) -> bool                                                                                                --- // append into the specified column. Return true when column is visible.
	...
	@(link_name="ImGui_TableSetupScrollFreeze") TableSetupScrollFreeze :: proc(cols: i32, rows: i32)                                                                            --- // lock columns/rows so they stay visible when scrolled.
	...
	@(link_name="ImGui_TableGetColumnCount")   TableGetColumnCount   :: proc() -> i32                                               --- // return number of columns (value passed to BeginTable)
	@(link_name="ImGui_TableGetColumnIndex")   TableGetColumnIndex   :: proc() -> i32                                               --- // return current column index.
	@(link_name="ImGui_TableGetRowIndex")      TableGetRowIndex      :: proc() -> i32                                               --- // return current row index (header rows are accounted for)
	@(link_name="ImGui_TableGetColumnName")    TableGetColumnName    :: proc(column_n: i32 = -1) -> cstring                         --- // return "" if column didn't have a name declared by TableSetupColumn(). Pass -1 to use current column.
	@(link_name="ImGui_TableGetColumnFlags")   TableGetColumnFlags   :: proc(column_n: i32 = -1) -> TableColumnFlags                --- // return column flags so you can query their Enabled/Visible/Sorted/Hovered status flags. Pass -1 to use current column.
	@(link_name="ImGui_TableSetColumnEnabled") TableSetColumnEnabled :: proc(column_n: i32, v: bool)                                --- // change user accessible enabled/disabled state of a column. Set to false to hide the column. User can use the context menu to change this themselves (right-click in headers, or right-click in columns body with ImGuiTableFlags_ContextMenuInBody)
	@(link_name="ImGui_TableGetHoveredColumn") TableGetHoveredColumn :: proc() -> i32                                               --- // return hovered column. return -1 when table is not hovered. return columns_count if the unused space at the right of visible columns is hovered. Can also use (TableGetColumnFlags() & ImGuiTableColumnFlags_IsHovered) instead.
	@(link_name="ImGui_TableSetBgColor")       TableSetBgColor       :: proc(target: TableBgTarget, color: u32, column_n: i32 = -1) --- // change the color of a cell, row, or column. See ImGuiTableBgTarget_ flags for details.
	// Legacy Columns API (prefer using Tables!)
	// - You can also use SameLine(pos_x) to mimic simplified columns.
	@(link_name="ImGui_Columns")         Columns         :: proc(count: i32 = 1, id: cstring = nil, borders: bool = true) ---
	...
	@(link_name="ImGui_GetColumnIndex")  GetColumnIndex  :: proc() -> i32                                                 --- // get current column index
	@(link_name="ImGui_GetColumnWidth")  GetColumnWidth  :: proc(column_index: i32 = -1) -> f32                           --- // get column width (in pixels). pass -1 to use current column
	@(link_name="ImGui_SetColumnWidth")  SetColumnWidth  :: proc(column_index: i32, width: f32)                           --- // set column width (in pixels). pass -1 to use current column
	@(link_name="ImGui_GetColumnOffset") GetColumnOffset :: proc(column_index: i32 = -1) -> f32                           --- // get position of column line (in pixels, from the left side of the contents region). pass -1 to use current column, otherwise 0..GetColumnsCount() inclusive. column 0 is typically 0.0f
	@(link_name="ImGui_SetColumnOffset") SetColumnOffset :: proc(column_index: i32, offset_x: f32)                        --- // set position of column line (in pixels, from the left side of the contents region). pass -1 to use current column
	@(link_name="ImGui_GetColumnsCount") GetColumnsCount :: proc() -> i32                                                 ---
	// Tab Bars, Tabs
	...
	@(link_name="ImGui_LogToTTY")           LogToTTY           :: proc(auto_open_depth: i32 = -1)                          --- // start logging to tty (stdout)
	@(link_name="ImGui_LogToFile")          LogToFile          :: proc(auto_open_depth: i32 = -1, filename: cstring = nil) --- // start logging to file
	@(link_name="ImGui_LogToClipboard")     LogToClipboard     :: proc(auto_open_depth: i32 = -1)                          --- // start logging to OS clipboard
	...
	@(link_name="ImGui_LogTextUnformatted") LogTextUnformatted :: proc(text: cstring)                                      --- // pass text data straight to log (without being displayed)
	// Drag and Drop
	...
	@(link_name="ImGui_SetDragDropPayload")    SetDragDropPayload    :: proc(type: cstring, data: rawptr, sz: uint, cond: Cond = {}) -> bool --- // type is a user defined string of maximum 32 characters. Strings starting with '_' are reserved for dear imgui internal types. Data is copied and held by imgui. Return true when payload has been accepted.
	...
	@(link_name="ImGui_SetKeyboardFocusHere") SetKeyboardFocusHere :: proc(offset: i32 = 0) --- // focus keyboard on the next widget. Use positive 'offset' to access sub components of a multiple component widget. Use -1 to access previous widget.
	...
	@(link_name="ImGui_GetFrameCount")         GetFrameCount         :: proc() -> i32                                --- // get global imgui frame count. incremented by 1 every frame.
	...
	@(link_name="ImGui_GetKeyPressedAmount")             GetKeyPressedAmount             :: proc(key: Key, repeat_delay: f32, rate: f32) -> i32 --- // uses provided repeat rate/delay. return a count, most often 0 or 1 but might be >1 if RepeatRate is small enough that DeltaTime > RepeatRate
	...
	@(link_name="ImGui_SetItemKeyOwner") SetItemKeyOwner :: proc(key: Key) -> bool --- // Set key owner to last item ID if it is hovered or active. Return true when ownership has been set. Roughly equivalent to 'if (TestKeyOwner(key, GetItemID()) && (IsItemHovered() || IsItemActive())) { SetKeyOwner(key, GetItemID());'. 
	...
	@(link_name="ImGui_GetMouseClickedCount")             GetMouseClickedCount             :: proc(button: MouseButton) -> i32                                   --- // return the number of successive mouse-clicks at the time where a click happen (otherwise 0).
	...
	@(link_name="ImGui_LoadIniSettingsFromMemory") LoadIniSettingsFromMemory :: proc(ini_data: cstring, ini_size: uint = 0) --- // call after CreateContext() and before the first call to NewFrame() to provide .ini data from your own data source.
	...
	@(link_name="ImGui_SaveIniSettingsToMemory")   SaveIniSettingsToMemory   :: proc(out_ini_size: ^uint = nil) -> cstring  --- // return a zero-terminated string with the .ini data which you can save by your own mean. call when io.WantSaveIniSettings is set, then save data by your own mean and clear io.WantSaveIniSettings.
	// Debug Utilities
	...
	@(link_name="ImGui_DebugCheckVersionAndDataLayout") DebugCheckVersionAndDataLayout :: proc(version_str: cstring, sz_io: uint, sz_style: uint, sz_vec2: uint, sz_vec4: uint, sz_drawvert: uint, sz_drawidx: uint) -> bool --- // This is called by IMGUI_CHECKVERSION() macro.
	...
	@(link_name="ImGui_DebugLogUnformatted")            DebugLogUnformatted            :: proc(text: cstring)                                                                                                                --- // Call via IMGUI_DEBUG_LOG() for maximum stripping in caller code!
	// Memory Allocators
	...
	@(link_name="ImGui_MemAlloc")              MemAlloc              :: proc(size: uint) -> rawptr                                                         ---
	...
	@(link_name="ImGuiStyle_ScaleAllSizes")                              Style_ScaleAllSizes                              :: proc(self: ^Style, scale_factor: f32)                                        --- // Scale all spacing/padding/thickness values. Do not scale fonts. See comments in definition. Consider not calling this if your initial scale factor if <1.0.
	// Input Functions
	...
	@(link_name="ImGuiIO_AddMouseButtonEvent")               IO_AddMouseButtonEvent               :: proc(self: ^IO, button: i32, down: bool)                                                            --- // Queue a mouse button change
	...
	@(link_name="ImGuiIO_AddInputCharacter")                 IO_AddInputCharacter                 :: proc(self: ^IO, _c: u32)                                                                            --- // Queue a new character input
	...
	@(link_name="ImGuiIO_SetKeyEventNativeData")             IO_SetKeyEventNativeData             :: proc(self: ^IO, key: Key, native_keycode: i32, native_scancode: i32, native_legacy_index: i32 = -1) --- // [Optional] Specify index for legacy <1.87 IsKeyXXX() functions with native indices + specify native keycode, scancode.
	...
	@(link_name="ImGuiInputTextCallbackData_DeleteChars")    InputTextCallbackData_DeleteChars    :: proc(self: ^InputTextCallbackData, pos: i32, bytes_count: i32)                                      ---
	@(link_name="ImGuiInputTextCallbackData_InsertChars")    InputTextCallbackData_InsertChars    :: proc(self: ^InputTextCallbackData, pos: i32, text: cstring, text_end: cstring = nil)                ---
	@(link_name="ImGuiInputTextCallbackData_SelectAll")      InputTextCallbackData_SelectAll      :: proc(self: ^InputTextCallbackData)                                                                  ---
	@(link_name="ImGuiInputTextCallbackData_SetSelection")   InputTextCallbackData_SetSelection   :: proc(self: ^InputTextCallbackData, s: i32, e: i32)                                                  ---
	...
	@(link_name="ImGuiTextFilter_ImGuiTextRange_split")      TextFilter_ImGuiTextRange_split      :: proc(self: ^TextFilter_ImGuiTextRange, separator: u8, out: ^Vector_TextRange)                       ---
	...
	@(link_name="ImGuiTextBuffer_size")                      TextBuffer_size                      :: proc(self: ^TextBuffer) -> i32                                                                      ---
	...
	@(link_name="ImGuiTextBuffer_resize")                    TextBuffer_resize                    :: proc(self: ^TextBuffer, size: i32)                                                                  --- // Similar to resize(0) on ImVector: empty string but don't free buffer.
	@(link_name="ImGuiTextBuffer_reserve")                   TextBuffer_reserve                   :: proc(self: ^TextBuffer, capacity: i32)                                                              ---
	...
	@(link_name="ImGuiStorage_GetInt")     Storage_GetInt     :: proc(self: ^Storage, key: ID, default_val: i32 = 0) -> i32       ---
	@(link_name="ImGuiStorage_SetInt")     Storage_SetInt     :: proc(self: ^Storage, key: ID, val: i32)                          ---
	...
	@(link_name="ImGuiStorage_GetIntRef")     Storage_GetIntRef     :: proc(self: ^Storage, key: ID, default_val: i32 = 0) -> ^i32         ---
	...
	@(link_name="ImGuiStorage_SetAllInt") Storage_SetAllInt :: proc(self: ^Storage, val: i32)                                       ---
	@(link_name="ImGuiListClipper_Begin") ListClipper_Begin :: proc(self: ^ListClipper, items_count: i32, items_height: f32 = -1.0) ---
	...
	@(link_name="ImGuiListClipper_IncludeItemByIndex")  ListClipper_IncludeItemByIndex  :: proc(self: ^ListClipper, item_index: i32)                ---
	@(link_name="ImGuiListClipper_IncludeItemsByIndex") ListClipper_IncludeItemsByIndex :: proc(self: ^ListClipper, item_begin: i32, item_end: i32) --- // item_end is exclusive e.g. use (42, 42+1) to make item 42 never clipped.
	// Seek cursor toward given item. This is automatically called while stepping.
	// - The only reason to call this is: you can use ImGuiListClipper::Begin(INT_MAX) if you don't know item count ahead of time.
	// - In this case, after all steps are done, you'll want to call SeekCursorForItem(item_count).
	@(link_name="ImGuiListClipper_SeekCursorForItem") ListClipper_SeekCursorForItem :: proc(self: ^ListClipper, item_index: i32) ---
	...
	@(link_name="ImGuiSelectionBasicStorage_GetStorageIdFromIndex") SelectionBasicStorage_GetStorageIdFromIndex :: proc(self: ^SelectionBasicStorage, idx: i32) -> ID                          --- // Convert index to item id based on provided adapter.
	...
	@(link_name="ImDrawListSplitter_Split")             DrawListSplitter_Split             :: proc(self: ^DrawListSplitter, draw_list: ^DrawList, count: i32)                                                 ---
	@(link_name="ImDrawListSplitter_Merge")             DrawListSplitter_Merge             :: proc(self: ^DrawListSplitter, draw_list: ^DrawList)                                                             ---
	@(link_name="ImDrawListSplitter_SetCurrentChannel") DrawListSplitter_SetCurrentChannel :: proc(self: ^DrawListSplitter, draw_list: ^DrawList, channel_idx: i32)                                           ---
	...
	@(link_name="ImDrawList_AddLineH")                DrawList_AddLineH                :: proc(self: ^DrawList, min_x: f32, max_x: f32, y: f32, col: u32, thickness: f32 = 1.0)                                                                                         ---
	@(link_name="ImDrawList_AddLineV")                DrawList_AddLineV                :: proc(self: ^DrawList, x: f32, min_y: f32, max_y: f32, col: u32, thickness: f32 = 1.0)                                                                                         ---
	@(link_name="ImDrawList_AddRect")                 DrawList_AddRect                 :: proc(self: ^DrawList, p_min: Vec2, p_max: Vec2, col: u32, rounding: f32 = 0.0, thickness: f32 = 1.0, flags: DrawFlags = {})                                                   --- // a: upper-left, b: lower-right (== upper-left + size)
	...
	@(link_name="ImDrawList_AddCircle")               DrawList_AddCircle               :: proc(self: ^DrawList, center: Vec2, radius: f32, col: u32, num_segments: i32 = 0, thickness: f32 = 1.0)                                                                       ---
	@(link_name="ImDrawList_AddCircleFilled")         DrawList_AddCircleFilled         :: proc(self: ^DrawList, center: Vec2, radius: f32, col: u32, num_segments: i32 = 0)                                                                                             ---
	@(link_name="ImDrawList_AddNgon")                 DrawList_AddNgon                 :: proc(self: ^DrawList, center: Vec2, radius: f32, col: u32, num_segments: i32, thickness: f32 = 1.0)                                                                           ---
	@(link_name="ImDrawList_AddNgonFilled")           DrawList_AddNgonFilled           :: proc(self: ^DrawList, center: Vec2, radius: f32, col: u32, num_segments: i32)                                                                                                 ---
	@(link_name="ImDrawList_AddEllipse")              DrawList_AddEllipse              :: proc(self: ^DrawList, center: Vec2, radius: Vec2, col: u32, rot: f32 = 0.0, num_segments: i32 = 0, thickness: f32 = 1.0)                                                      ---
	@(link_name="ImDrawList_AddEllipseFilled")        DrawList_AddEllipseFilled        :: proc(self: ^DrawList, center: Vec2, radius: Vec2, col: u32, rot: f32 = 0.0, num_segments: i32 = 0)                                                                            ---
	...
	@(link_name="ImDrawList_AddBezierCubic")          DrawList_AddBezierCubic          :: proc(self: ^DrawList, p1: Vec2, p2: Vec2, p3: Vec2, p4: Vec2, col: u32, thickness: f32, num_segments: i32 = 0)                                                                --- // Cubic Bezier (4 control points)
	@(link_name="ImDrawList_AddBezierQuadratic")      DrawList_AddBezierQuadratic      :: proc(self: ^DrawList, p1: Vec2, p2: Vec2, p3: Vec2, col: u32, thickness: f32, num_segments: i32 = 0)                                                                          --- // Quadratic Bezier (3 control points)
	// General polygon
	// - Only simple polygons are supported by filling functions (no self-intersections, no holes).
	// - Concave polygon fill is more expensive than convex one: it has O(N^2) complexity. Provided as a convenience for the user but not used by the main library.
	@(link_name="ImDrawList_AddPolyline")          DrawList_AddPolyline          :: proc(self: ^DrawList, points: ^Vec2, num_points: i32, col: u32, thickness: f32, flags: DrawFlags = {}) ---
	@(link_name="ImDrawList_AddConvexPolyFilled")  DrawList_AddConvexPolyFilled  :: proc(self: ^DrawList, points: ^Vec2, num_points: i32, col: u32)                                        ---
	@(link_name="ImDrawList_AddConcavePolyFilled") DrawList_AddConcavePolyFilled :: proc(self: ^DrawList, points: ^Vec2, num_points: i32, col: u32)                                        ---
	// Image primitives
	...
	@(link_name="ImDrawList_PathStroke")                 DrawList_PathStroke                 :: proc(self: ^DrawList, col: u32, thickness: f32 = 1.0, flags: DrawFlags = {})                               ---
	@(link_name="ImDrawList_PathArcTo")                  DrawList_PathArcTo                  :: proc(self: ^DrawList, center: Vec2, radius: f32, a_min: f32, a_max: f32, num_segments: i32 = 0)            ---
	@(link_name="ImDrawList_PathArcToFast")              DrawList_PathArcToFast              :: proc(self: ^DrawList, center: Vec2, radius: f32, a_min_of_12: i32, a_max_of_12: i32)                       --- // Use precomputed angles for a 12 steps circle
	@(link_name="ImDrawList_PathEllipticalArcTo")        DrawList_PathEllipticalArcTo        :: proc(self: ^DrawList, center: Vec2, radius: Vec2, rot: f32, a_min: f32, a_max: f32, num_segments: i32 = 0) --- // Ellipse
	@(link_name="ImDrawList_PathBezierCubicCurveTo")     DrawList_PathBezierCubicCurveTo     :: proc(self: ^DrawList, p2: Vec2, p3: Vec2, p4: Vec2, num_segments: i32 = 0)                                 --- // Cubic Bezier (4 control points)
	@(link_name="ImDrawList_PathBezierQuadraticCurveTo") DrawList_PathBezierQuadraticCurveTo :: proc(self: ^DrawList, p2: Vec2, p3: Vec2, num_segments: i32 = 0)                                           --- // Quadratic Bezier (3 control points)
	...
	@(link_name="ImDrawList_AddCallback") DrawList_AddCallback :: proc(self: ^DrawList, callback: DrawCallback, userdata: rawptr = nil, userdata_size: uint = 0) ---
	...
	@(link_name="ImDrawList_ChannelsSplit")      DrawList_ChannelsSplit      :: proc(self: ^DrawList, count: i32) ---
	@(link_name="ImDrawList_ChannelsMerge")      DrawList_ChannelsMerge      :: proc(self: ^DrawList)             ---
	@(link_name="ImDrawList_ChannelsSetCurrent") DrawList_ChannelsSetCurrent :: proc(self: ^DrawList, n: i32)     ---
	// Advanced: Primitives allocations
	// - We render triangles (three vertices)
	// - All primitives needs to be reserved via PrimReserve() beforehand.
	@(link_name="ImDrawList_PrimReserve")   DrawList_PrimReserve   :: proc(self: ^DrawList, idx_count: i32, vtx_count: i32)                                                                ---
	@(link_name="ImDrawList_PrimUnreserve") DrawList_PrimUnreserve :: proc(self: ^DrawList, idx_count: i32, vtx_count: i32)                                                                ---
	...
	@(link_name="ImDrawList__CalcCircleAutoSegmentCount") DrawList__CalcCircleAutoSegmentCount :: proc(self: ^DrawList, radius: f32) -> i32                                                           ---
	@(link_name="ImDrawList__PathArcToFastEx")            DrawList__PathArcToFastEx            :: proc(self: ^DrawList, center: Vec2, radius: f32, a_min_sample: i32, a_max_sample: i32, a_step: i32) ---
	@(link_name="ImDrawList__PathArcToN")                 DrawList__PathArcToN                 :: proc(self: ^DrawList, center: Vec2, radius: f32, a_min: f32, a_max: f32, num_segments: i32)         ---
	...
	@(link_name="ImTextureData_Create")                   TextureData_Create                   :: proc(self: ^TextureData, format: TextureFormat, w: i32, h: i32)                                     ---
	...
	@(link_name="ImTextureData_GetPixelsAt")              TextureData_GetPixelsAt              :: proc(self: ^TextureData, x: i32, y: i32) -> rawptr                                                  ---
	@(link_name="ImTextureData_GetSizeInBytes")           TextureData_GetSizeInBytes           :: proc(self: ^TextureData) -> i32                                                                     ---
	@(link_name="ImTextureData_GetPitch")                 TextureData_GetPitch                 :: proc(self: ^TextureData) -> i32                                                                     ---
	...
	@(link_name="ImFontGlyphRangesBuilder_GetBit")                  FontGlyphRangesBuilder_GetBit                  :: proc(self: ^FontGlyphRangesBuilder, n: uint) -> bool                                                                                                                           --- // Get bit n in the array
	@(link_name="ImFontGlyphRangesBuilder_SetBit")                  FontGlyphRangesBuilder_SetBit                  :: proc(self: ^FontGlyphRangesBuilder, n: uint)                                                                                                                                   --- // Set bit n in the array
	...
	@(link_name="ImFontAtlas_Clear")                                FontAtlas_Clear                                :: proc(self: ^FontAtlas)                                                                                                                                                         --- // Clear everything (fonts + textures). Don't call mid-frame!
	@(link_name="ImFontAtlas_ClearFonts")                           FontAtlas_ClearFonts                           :: proc(self: ^FontAtlas)                                                                                                                                                         --- // Clear input+output font data/glyphs. You can call this mid-frame if you load new fonts afterwards!
	...
	@(link_name="ImFontAtlas_AddCustomRect")       FontAtlas_AddCustomRect       :: proc(self: ^FontAtlas, width: i32, height: i32, out_r: ^FontAtlasRect = nil) -> FontAtlasRectId --- // Register a rectangle. Return -1 (ImFontAtlasRectId_Invalid) on error.
	...
	@(link_name="ImFont_IsGlyphRangeUnused") Font_IsGlyphRangeUnused :: proc(self: ^Font, c_begin: u32, c_last: u32) -> bool          ---
	// Helpers
...
// Scalar data types
ID       :: u32 // A unique ID used by widgets (typically the result of hashing a stack of string)
KeyChord :: i32 // -> ImGuiKey | ImGuiMod_XXX    // Flags: for IsKeyChordPressed(), Shortcut() etc. an ImGuiKey optionally OR-ed with one or more ImGuiMod_XXX values.
// Character types
...
InputTextCallback :: proc "c" (data: ^InputTextCallbackData) -> i32   // Callback function for ImGui::InputText()
SizeCallback      :: proc "c" (data: ^SizeCallbackData)               // Callback function for ImGui::SetNextWindowSizeConstraints()
MemAllocFunc      :: proc "c" (sz: uint, user_data: rawptr) -> rawptr // Function signature for ImGui::SetAllocatorFunctions()
